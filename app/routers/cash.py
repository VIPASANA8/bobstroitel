...
from cash.referrals import bot_link, summary as referral_summary, web_link
...
from cash.withdrawals import (
    MIN_RUB_PAYOUT_KOPECKS, P2P_RUB, ActiveWithdrawalExists, RubRateUnset, WithdrawalStateError,
)
...
async def wallet(request: Request, user: AuthenticatedUser = Depends(get_cash_user)):
    ...
    return {
        **payload,
        ...
        "rub_withdrawal_fee_usdt": micros_to_usdt(withdrawals.fee_for(P2P_RUB)),
    }
...
async def referral(request: Request, user: AuthenticatedUser = Depends(get_current_user)):
    ...
    return {
        **data,
        "link": bot_link(username, data["code"]) if username else None,
