...
### Устранение двух исходных падений тестов — 2026-08-31

Оба сбоя выше относились к устаревшим тестам и закрыты отдельно от удаления
слоёв. `test_chat_returns_only_last_fifty_messages` теперь создаёт сообщения
с разными свежими метками времени внутри часового срока хранения и сверяет
весь ожидаемый список из 50 сообщений. Раньше `now=0..54` датировал их
1970 годом, поэтому `recent()` правильно возвращал пустой список.

`test_the_countdown_runs_out_instead_of_starting_over` удалён вместе с
устаревшим требованием к текстовой строке: саму строку и `countdownText`
убрал коммит `b80d782`. Действующее кольцо на аватаре остаётся покрыто
`test_ready_countdown_visible_while_waiting.py` (выбор срока отсчёта) и
`test_the_ring_measures_against_the_whole_countdown` (повторный снимок
не сбрасывает длительность). Рабочий код приложения не менялся.

Повторный полный небраузерный прогон: **678 прошли, 3 пропущены,
1 исключён по маркеру; падений нет** (`pytest --ignore=tests/e2e -q`).
