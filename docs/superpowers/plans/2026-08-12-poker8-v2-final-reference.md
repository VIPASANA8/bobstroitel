...
**Goal:** Finish the real mobile Poker8 screen to match the approved cinematic reference, including a perspective table, full reference-style seats, a lowered slider row, a stable four-action HUD, and an untouched bottom reserve.
...
- [ ] Restyle the existing action panel into a fixed-height front-facing HUD with summary row, five bright preset columns, amount controls, slider, and four equal primary actions in a 2 × 2 grid.
- [ ] Add dynamic FOLD/CHECK and BET/RAISE labels, persistent disabled slots, three-second ALL-IN confirmation, MAX-to-ALL-IN handling, and reduced-motion behavior.
- [ ] Keep the existing menu and decorative chat unchanged, and keep the separate bottom reserve visually empty and non-interactive.
...
- [ ] Inspect pre-hand and active-hand layouts at 360 × 800 and 402 × 874: all six seats, pot, board, chips, hero cards, lowered slider, 2 × 2 HUD actions, and empty bottom reserve fit without document scroll or clipping.
