...
**Goal:** Finish the real mobile Poker8 screen to match the approved cinematic reference, including a perspective table, full reference-style seats, a lowered slider row, three primary actions in the HUD's empty lower area, and an untouched bottom reserve.
...
- [ ] Restyle the existing action panel into a fixed-height front-facing HUD with summary row, five preset columns, amount controls, slider in the former action row, and three equal primary actions in the HUD's empty lower area.
...
- [ ] Inspect pre-hand and active-hand layouts at 360 × 800 and 402 × 874: all six seats, pot, board, chips, hero cards, lowered slider, lower HUD actions, and empty bottom reserve fit without document scroll or clipping.
