...
- Move FOLD/CALL/RAISE lower only inside the currently empty part of the HUD. Keep the separate bottom reserve completely empty for a future global application menu.
...
At 360 × 800, the fixed header occupies the top safe strip, the cinematic table fills the upper game region, and the HUD sits below it without vertical scrolling. The upper seat, hero seat, community cards, pot, chips, wagers, and all five opponent seats stay fully visible. The slider occupies the former action row, the three primary actions use the empty lower part of the HUD, and the separate bottom reserve remains empty.
