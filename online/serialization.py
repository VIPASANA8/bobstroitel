...
def serialize_state(state: GameState) -> dict:
    return {
        ...
        "raise_capped": sorted(state.raise_capped),
        "rake": state.rake,
        "rake_by_player": dict(state.rake_by_player),
        "winner": state.winner,
...
def deserialize_state(payload: dict) -> GameState:
    ...
    return GameState(
        ...
        rake_by_player=dict(payload.get("rake_by_player") or {}),
        ...
        raise_capped=set(payload.get("raise_capped", [])),
