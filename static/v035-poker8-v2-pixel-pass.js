(() => {
  ...
  function syncPixelPassText() {
    ...
    if (heroCard) {
      ...
      if (avatar && (!window.game || window.game?.terminal)) avatar.textContent = "ВЫ";
    }

    document.querySelectorAll("#actionButtons button").forEach((button) => {
      const next = toEnglishBb(button.textContent);
      if (next !== button.textContent) button.textContent = next;
    });

    document.querySelectorAll(".quick-sizes button").forEach((button) => {
      if (button.dataset.sizing === "1.00") button.textContent = "POT";
    });

    const sizingLabel = document.querySelector("#sizingWrap > label");
    if (sizingLabel) sizingLabel.textContent = "BET SIZE";

    const minLabel = document.getElementById("sliderMinLabel");
    const maxLabel = document.getElementById("sliderMaxLabel");
    if (minLabel) minLabel.textContent = toEnglishBb(minLabel.textContent);
    if (maxLabel) maxLabel.textContent = toEnglishBb(maxLabel.textContent);
  }
  ...
  const observer = new MutationObserver(() => {
    if (isV2()) requestAnimationFrame(syncPixelPassText);
  });
  ...
  style.textContent = `
    @media (max-width:780px){
      ...
      /* Fix the real screenshot bug: identity was behind the v033 glass overlay. */
      ...
      body.v014.poker8-v2-sixmax .seat{
      ...
      body.v014.poker8-v2-sixmax .seat[data-visual-seat="0"] .seat-stack{
        color:#35bfff!important;font-size:14px!important;
      }
      ...
      /* Readiness remains useful, but stops competing with names/stacks. */
      ...
      body.v014.poker8-v2-sixmax .pot-total-label{
        color:#9bc5b5!important;font-size:8px!important;letter-spacing:.10em!important;
      }
      body.v014.poker8-v2-sixmax .pot-total strong{
        color:#fff!important;font-size:21px!important;text-shadow:0 0 9px rgba(72,239,180,.16)!important;
      }

      body.v014.poker8-v2-sixmax .v028-center-ready{
        top:56%!important;width:158px!important;gap:7px!important;
      }
      ...
      body.v014.poker8-v2-sixmax .action-grid .action-slot{
        min-height:46px!important;border-radius:11px!important;font-size:10px!important;
      }
      ...
      body.v014.poker8-v2-sixmax .amount-row{
        min-height:50px!important;margin-top:14px!important;padding:4px!important;border-radius:13px!important;
      }
      ...
      body.v014.poker8-v2-sixmax .amount-row::before{
        content:"BET SIZE"!important;color:#6f8f89!important;font-size:6px!important;letter-spacing:.13em!important;
      }
      ...
      /* Board remains the brightest information object once a hand starts. */
      body.v014.poker8-v2-sixmax .board-cards{top:49%!important;gap:4px!important}
      body.v014.poker8-v2-sixmax .board-cards .card{
        width:45px!important;height:64px!important;font-size:16px!important;
      }
    }
