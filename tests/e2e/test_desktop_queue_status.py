"""The pending-seat message belongs in the desktop header, not on a player."""
...
def test_queue_status_moves_between_desktop_header_and_mobile_felt(online_server):
    with sync_playwright() as playwright:
        ...
        try:
            ...
            for width in (1485, 1192, 781):
                ...
                assert panel.locator("..").get_attribute("class") == "topbar"
                ...
                assert box and header and brand and actions
                assert box["x"] >= brand["x"] + brand["width"] - 1
                assert box["x"] + box["width"] <= actions["x"] + 1
            ...
            page.wait_for_timeout(150)
            # Mobile already reports the queue in its header/seat control;
            # the old panel remains hidden there, exactly as before.
